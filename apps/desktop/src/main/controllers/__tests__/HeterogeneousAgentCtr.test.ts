...
describe('HeterogeneousAgentCtr', () => {
  ...
  describe('sendPrompt (pi rpc)', () => {
    ...
    it('moves the thinking level on a reused process instead of spawning fresh', async () => {
      const send = vi.fn();
      ...
      // Same conversation, only the level changed: the warm process must be
      // reused and the level moved over RPC instead of paying a respawn.
      const second = await ctr.startSession({
        agentType: 'pi',
        args: ['--thinking', 'high'],
        command: 'pi',
      ...
      expect(piRpcSessionConstructMock).toHaveBeenCalledTimes(1);
      expect(piRpcSessionSetThinkingLevelMock).toHaveBeenCalledWith('high');
    });
