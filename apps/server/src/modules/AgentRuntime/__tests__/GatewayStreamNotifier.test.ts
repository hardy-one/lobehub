...
describe('GatewayStreamNotifier', () => {
  ...
  describe('publishAgentRuntimeInit', () => {
    ...
    it('keeps init, step_start, and stream_start in delivery order', async () => {
      ...
      const initResult = notifier.publishAgentRuntimeInit('op-1', { userId: 'user-1' });
      await flush();
      const stepResult = notifier.publishStreamEvent('op-1', {
        data: { uiMessages: [] },
        stepIndex: 0,
        type: 'step_start',
      });
      const streamResult = notifier.publishStreamEvent('op-1', {
        data: { assistantMessage: { id: 'assistant-1' } },
        stepIndex: 0,
        type: 'stream_start',
      });
      await Promise.all([stepResult, streamResult]);
      await flush();
      ...
      resolveNext();
      await expect(initResult).resolves.toBe('publishAgentRuntimeInit-result');
      await flush();
