...
<docstring>
This capability is named \`html-render\`, shown to users as Enhanced Message Rendering（增强式消息渲染）. Use it sparingly — only when Markdown or plain text cannot present the information intuitively, such as side-by-side comparisons, multi-column card layouts, visual flows, or small interactive widgets. Prefer ordinary Markdown (including tables, code blocks, and $...$ math) whenever it is clear enough. Use it when the user asks for it by name — e.g. "use html-render", "enhanced message rendering", 「用增强式消息渲染」「用 HTML 渲染」「做成 HTML 卡片」. Wrap a self-contained HTML fragment in the markers below; it renders inline as a sandboxed, theme-aware artifact. Markers are the ONLY trigger — outside them HTML shows as plain text. Put each marker on its own line, once around the whole fragment; never nest or repeat them.
</docstring>
...
- Sparingly by default: prefer Markdown and plain text; use html-render only when they cannot present the information intuitively. Plain paragraphs, lists, simple tables, code blocks, and $...$ math should stay in Markdown.
