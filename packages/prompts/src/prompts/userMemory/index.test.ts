...
describe('promptUserMemory lean mode', () => {
  const persona = {
    persona: {
      narrative: `## Hardy — researcher

Intro line about the persona.

### Identity

A researcher in optimization.

### What you care about

Long research details that should be dropped in lean mode.

### Interaction cues

Prefers concise technical answers with equations first.

### Goals and risks

Current goals that should be dropped in lean mode.`,
      tagline: 'Hardy — the researcher',
    },
  };

  it('lean mode keeps only Identity and Interaction cues sections', () => {
    const result = promptUserMemory({ memories: persona }, true);
    expect(result).toContain('### Identity');
    expect(result).toContain('A researcher in optimization.');
    expect(result).toContain('### Interaction cues');
    expect(result).toContain('Prefers concise technical answers');
    expect(result).not.toContain('What you care about');
    expect(result).not.toContain('Long research details');
    expect(result).not.toContain('Goals and risks');
    expect(result).not.toContain('Current goals');
  });

  it('lean mode keeps tagline', () => {
    const result = promptUserMemory({ memories: persona }, true);
    expect(result).toContain('tagline="Hardy — the researcher"');
  });

  it('lean mode degrades safely when narrative has no sections', () => {
    const plain = {
      persona: { narrative: 'Just a plain narrative without markdown sections.', tagline: null },
    };
    const result = promptUserMemory({ memories: plain }, true);
    expect(result).toContain('Just a plain narrative');
  });
});
