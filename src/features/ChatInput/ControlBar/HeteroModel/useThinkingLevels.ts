...
export const useThinkingLevels = ({
  ...
  return {
    /**
     * Effective level the probe session sits on — pi's own default, or the
     * clamp of a level the model does not serve.
     */
    effectiveLevel: result?.status === 'success' ? result.thinkingLevel : undefined,
