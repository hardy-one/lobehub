...
> = (set, get) => ({
  ...
  refreshDocuments: async (topicId) => {
    // Directly fetch and update the store since SWR mutate may not work
    // when the component using useFetchDocuments is not mounted
    const result = await notebookService.listDocuments({ topicId });

    set(
      {
        notebookMap: { ...get().notebookMap, [topicId]: result.data },
      },
      false,
      n('refreshDocuments', { topicId }),
    );

    // Also trigger SWR mutate for any mounted components
    await mutate(
      (key) =>
        Array.isArray(key) && key[0] === SWR_USE_FETCH_NOTEBOOK_DOCUMENTS && key[1] === topicId,
      result.data,
      { revalidate: false },
    );
  },
